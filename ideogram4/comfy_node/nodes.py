"""ComfyUI nodes that render on a remote Ideogram 4 deployment.

Drop this package into `ComfyUI/custom_nodes/` on the machine you actually work
on. The nodes hold no weights: they post to the Modal endpoint's `/generate`
contract and return the decoded image as a normal IMAGE tensor, so they compose
...
def _endpoint(override: str) -> str:
    url = (override or _setting("IDEOGRAM4_MODAL_URL")).strip().rstrip("/")
    if not url:
        raise RuntimeError(
            "No endpoint configured. Set IDEOGRAM4_MODAL_URL (or fill the "
...
def _post(url: str, path: str, payload: dict[str, Any], timeout: float) -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(f"Ideogram 4 endpoint unreachable at {url}: {exc}") from exc
    if response.status_code >= 400:
        raise RuntimeError(
            f"Ideogram 4 endpoint returned {response.status_code}: {response.text[:2000]}"
        )
...
class _ProgressMirror:
    ...
    def __enter__(self) -> _ProgressMirror:
        self._thread = threading.Thread(target=self._run, name="ideogram4-progress", daemon=True)
        self._thread.start()
    ...
    def INPUT_TYPES(cls) -> dict[str, Any]:
        return {
            ...
            "optional": {
                "endpoint": (
                    "STRING",
                    {"default": "", "tooltip": "Overrides IDEOGRAM4_MODAL_URL."},
                ),
...
class Ideogram4ModalCaptionTemplate:
    ...
    def fetch(self, prompt: str, width: int, height: int, endpoint: str = ""):
        ...
        if response.status_code >= 400:
            raise RuntimeError(
                f"Ideogram 4 endpoint returned {response.status_code}: {response.text[:500]}"
            )
