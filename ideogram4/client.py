...
def endpoint(override: str | None) -> str:
    url = (override or os.environ.get("IDEOGRAM4_MODAL_URL", "")).strip().rstrip("/")
    if not url:
        sys.exit("set IDEOGRAM4_MODAL_URL or pass --url")
    return url
...
def cmd_generate(args: argparse.Namespace) -> None:
    ...
    print(f"rendering on {args.url or os.environ.get('IDEOGRAM4_MODAL_URL')} ...", flush=True)
    result = check(
        requests.post(
...
def cmd_template(args: argparse.Namespace) -> None:
    response = check(
        requests.get(
            f"{endpoint(args.url)}/caption-template",
            params={"prompt": args.prompt, "width": args.width, "height": args.height},
            headers=headers(),
            timeout=60,
        )
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--url", help="defaults to $IDEOGRAM4_MODAL_URL")
    ...
    tmpl = sub.add_parser("template", help="print the magic-prompt template for an LLM")
...
def main() -> None:
    ...
    if args.command == "generate" and not args.prompt and not args.json_prompt:
