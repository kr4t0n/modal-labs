...
def test_the_file_id_is_what_selects_the_precision():
    """This version publishes fp8, int8, int4 and nvfp4 under one filename.

    The version id identifies none of them. Only the file id says which is
    fetched, so it is pinned and asserted — the strongest case in this repo for
    not relying on Civitai's choice of primary file.
    """
    checkpoint = next(f for f in app.MODEL_FILES if isinstance(f, weights.CivitaiFile))
    assert checkpoint.model_version_id == 3139241
    assert checkpoint.file_id == 3019490
    assert f"fileId={checkpoint.file_id}" in checkpoint.url
    # The destination has to disambiguate too; upstream's name does not.
    assert "fp8" in checkpoint.destination


def test_no_civitai_secret_is_attached():
    """Verified anonymous: a ranged GET on the real URL returns 206.

    Asserted so the "needs no credentials" claim in the README stays honest —
    the same claim was wrong for finepornv4 and cost a release.
    """
...
def test_the_krea2_companions_match_the_ultra_service_exactly():
    """Four services, one base model, one pair of companion files.

    finepornv4 and redgpt2gpt carry the same assertion against ultra, so pinning
    each to ultra transitively pins all four. If these diverge it means one
    picked up a different encoder or VAE build, which changes output without
    failing.
    """
