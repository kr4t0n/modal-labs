"""Structural tests for the RedCraft v3 graph.

Guards the things that fail silently: a companion file that drifts away from the
other Krea 2 services, sampler defaults quietly replaced by a neighbouring
service's, or a link pointing at a renamed node. Whether the node schemas still
match a given ComfyUI build is answered by `client.py validate` against a live
deployment.
"""
...
def test_sampler_defaults_come_from_the_published_recipe():
    """The version notes read "ER_SDE / Euler | Simple | CFG =1 | 8-12 Steps".

    Unlike redgpt2gpt over the same base, this edition publishes settings, so
    these are the author's rather than a template fallback.
    """
...
def test_the_alternate_sampler_is_recorded_not_defaulted():
    """The card names two interchangeably; only one can be the default.

    `er_sde` is not certain to exist in every ComfyUI build, and a default that
    fails to resolve would break every request omitting a sampler — so it is
    offered rather than assumed.
    """
    assert workflow.ALTERNATE_SAMPLER == "er_sde"
    assert workflow.DEFAULT_SAMPLER != workflow.ALTERNATE_SAMPLER


def test_defaults_source_credits_the_card_not_a_template():
    assert "template" in workflow.DEFAULTS_SOURCE
    assert "not a ComfyUI template" in workflow.DEFAULTS_SOURCE
...
def test_the_loaded_file_names_its_precision():
    """Upstream gives four precisions one filename; the rename must not."""
    assert "fp8" in workflow.DIFFUSION_MODEL
