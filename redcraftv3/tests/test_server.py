"""RedCraft v3's slice of the ASGI layer.
...
@pytest.mark.asyncio
async def test_generate_submits_the_published_recipe(wired):
    ...
    assert submitted["sample"]["inputs"]["scheduler"] == "simple"
    assert submitted["positive"]["inputs"]["text"] == "a test"


@pytest.mark.asyncio
async def test_the_alternate_sampler_is_accepted(wired):
    """The card names er_sde first; it must be reachable even if not default."""
    _, client, state = wired
    await client.post("/generate", json={"prompt": "a test", "sampler_name": "er_sde"})
    assert state["prompts"][0]["prompt"]["sample"]["inputs"]["sampler_name"] == "er_sde"
...
@pytest.mark.asyncio
async def test_defaults_endpoint_credits_the_card_and_names_both_samplers(wired):
    _, client, _ = wired
    body = (await client.get("/defaults")).json()
    assert body["steps"] == 10 and body["cfg"] == 1.0
    assert body["steps_range"] == [8, 12]
    assert body["sampler_name"] == "euler" and body["scheduler"] == "simple"
    assert body["alternate_sampler"] == "er_sde"
    assert body["diffusion_model"] == workflow.DIFFUSION_MODEL
    # Attributed to the card, unlike redgpt2gpt over the same base.
    assert "not a ComfyUI template" in body["source"]
