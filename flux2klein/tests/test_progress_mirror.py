...
def load_node_module():
    ...
    spec = importlib.util.spec_from_file_location("flux2klein_node_under_test", NODE_SOURCE)
...
def test_generate_sends_a_client_id_matching_the_subscription(monkeypatch):
    ...
    def fake_post(url, path, payload, timeout):
        ...
        return {"images": [], "params": {"seed": 1, "variant": "base"}, "duration_s": 0.1}
    ...
    nodes.Flux2KleinModal().generate(
        prompt="a test",
        negative_prompt="",
        variant="base",
        aspect_ratio="1:1",
