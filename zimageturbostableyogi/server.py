"""Z-Image Turbo (Stable Yogi)'s model-specific slice of the shared ComfyUI service."""
...
class GenerateRequest(BaseGenerateRequest):
    """Z-Image sampler parameters, defaulted to the turbo reference settings."""
    ...
    shift: float = Field(
        default=workflow.DEFAULT_SHIFT,
        gt=0.0,
        le=100.0,
...
SERVICE = ModelService(
    title="Z-Image Turbo on ComfyUI",
    description="A ComfyUI server with the Stable Yogi Z-Image Turbo finetune, plus a typed generate endpoint.",
...
def _register_extra_routes(web_app: FastAPI) -> None:
    ...
    async def defaults() -> dict:
        ...
        return {
            ...
            "shift": workflow.DEFAULT_SHIFT,
            "source": "ComfyUI's official Z-Image Turbo template",
