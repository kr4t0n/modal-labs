"""RedCraft v3's model-specific slice of the shared ComfyUI service."""
...
class GenerateRequest(BaseGenerateRequest):
    """Krea 2 sampler parameters, defaulted to this edition's published recipe."""

    prompt: str = Field(description="Natural-language prompt.")
    negative_prompt: str = Field(
        default="",
        description=(
            "Only meaningful alongside a raised cfg: at the card's cfg 1 the "
...
SERVICE = ModelService(
    title="RedCraft v3 (Krea 2) on ComfyUI",
    description="A ComfyUI server with the RedCraft v3 Krea 2 finetune, plus a typed generate endpoint.",
...
def _register_extra_routes(web_app: FastAPI) -> None:
    ...
    async def defaults() -> dict:
        ...
        return {
            ...
            "alternate_sampler": workflow.ALTERNATE_SAMPLER,
